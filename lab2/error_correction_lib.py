...
def decode_syndrome_minLLR(y, s, s_y_joins, y_s_joins, qber_est, s_pos, p_pos, k_pos, 
                         r_start=None, max_iter=300, x=None, show=1, discl_n=20, n_iter_avg_window=5,
                         track_llr=False):
    """
    Modified to include 'track_llr'. If True, returns history of LLRs.
    """

    def h_func(x):
        if x < -3: return 0
        elif x < -0.68: return -0.25*x-0.75
        elif x < -0.27: return -2*x-1.94
        elif x < 0: return -8*x-3.56
        elif x < 0.15: return 16*x-4
        elif x < 0.4: return 4*x-2.2
        elif x < 1.3: return 2*x-1.4
        else: return x-0.1

    def core_func(x, y):
        # Piecewise approximation
        def g_func_piecewise(val):
            if val < 0.5: return -val/2+0.7
            elif val < 1.6: return -val/4+0.575
            elif val < 2.2: return -val/8+0.375
            elif val < 3.2: return -val/16+0.2375
            elif val < 4.4: return -val/32+0.1375
            else: return 0
        return sign(x)*sign(y)*min(abs(x), abs(y))+g_func_piecewise(abs(x+y))-g_func_piecewise(abs(x-y))
    ...
    while n_iter < max_iter:
        # Part 1: Check to Symbol
        E = np.zeros((m, n))
        for j in range(m):
            M_cur = M[j][s_y_joins[j]]
            M_cur_n = len(M_cur)
            n_zeros = list(M_cur).count(0.0)
            if n_zeros > 1:
                E[j, s_y_joins[j]] = np.zeros(M_cur_n)
            elif n_zeros == 1:
                ...
                M_cur.pop(zero_ind)
                LS = M_cur[0]
                for k in range(1, M_cur_n-1):
                    LS = core_func(LS, M_cur[k])
                E_cur[zero_ind] = LS
                E[j, s_y_joins[j]] = E_cur
            elif n_zeros == 0:
                LS = M_cur[0]
                ...
                for i1 in range(0, M_cur_n):
                    E[j][s_y_joins[j][i1]] = (1-2*s[j])*(h_func(M_cur[i1]+LS)-h_func(M_cur[i1]-LS)-LS)

        # Part 2: Symbol to Check
        sum_E = E.sum(axis=0)+r
        
        if track_llr:
            llr_history.append(sum_E.copy())

        z = (1-np.sign(sum_E))/2

        if (s == encode_syndrome(z, s_y_joins)).all():
            if show > 0:
                pass 
            return z, None, llr_history if track_llr else None
        ...
        if n_iter >= n_iter_avg_window:
            ...
            if sum_E_mean_avg_cur <= sum_E_mean_avg_old:
                ...
                return None, minLLR_inds, llr_history if track_llr else None
            else:
...
def perform_ec(x, y, s_y_joins, y_s_joins, qber_est, s_n, p_n, punct_list=None, discl_n=20, show=0, track_llr=False):
    n = len(y_s_joins)
    m = len(s_y_joins)
    ...
    e_pat, minLLR_inds, llr_hist = decode_syndrome_minLLR(
        e_pat_in, s_d, s_y_joins, y_s_joins, qber_est, s_pos, p_pos, k_pos, 
        max_iter=100500, x=key_sum, show=show, discl_n=discl_n, n_iter_avg_window=5,
        track_llr=track_llr
    )
    ...
    while e_pat is None:
        ...
        e_pat, minLLR_inds, _ = decode_syndrome_minLLR(
            e_pat_in, s_d, s_y_joins, y_s_joins, qber_est, s_pos, p_pos, k_pos, 
            r_start=None, max_iter=100500, x=key_sum, show=show, discl_n=discl_n, n_iter_avg_window=5,
            track_llr=False # Do not track LLRs for recursive steps to keep it simple
        )
        add_info += discl_n
        com_iters += 1
    ...
    return add_info, com_iters, x_dec, ver_check, llr_hist


def print_parameters_schema(qber_range, R_range, n, f=1.0):
    """
    Prints a schema table showing the chosen code parameters (Rate, s_n, p_n)
    for each QBER value in the provided range.
    """
    print(f"\n{'QBER':<8} | {'Base Rate':<10} | {'Shortened (s_n)':<16} | {'Punctured (p_n)':<16} | {'Effective Rate':<14}")
    print("-" * 75)
    
    for qber in qber_range:
        result = choose_sp(qber, f, R_range, n)
        
        if result is not None:
            R, s_n, p_n = result

            eff_R = (R - (s_n/n)) / (1 - s_n/n - p_n/n)
            
            print(f"{qber:<8.4f} | {R:<10.4f} | {s_n:<16d} | {p_n:<16d} | {eff_R:<14.4f}")
        else:
            print(f"{qber:<8.4f} | {'No Solution Found':<50}")
    print("-" * 75 + "\n")

def test_ec(qber, R_range, codes, n, n_tries, f_start=1, show=1, discl_k=1, error_func=None):
    ...
    for i in range(n_tries):
        ...
        y = error_func(x, qber)
        add_info, com_iters, x_dec, ver_check, _ = perform_ec(
